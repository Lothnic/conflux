...
export default function PlanningAnalysisPanel({ issue, threads, onClose }: PlanningAnalysisPanelProps) {
  ...
  // Reset scroll position when a different issue is opened.
  useEffect(() => {
  ...
  return (
    <aside className="fixed right-4 top-[72px] bottom-4 z-40 flex w-[456px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl">
      {/* Header (map, title, stats, citations) and detail sections share one scroll
          container; only the action bar below stays pinned. min-h-0 is required so
          the flex child can shrink and overflow-y-auto actually engages. */}
      ...
      <div className="flex shrink-0 gap-2 border-t border-slate-200 bg-white px-5 py-3">
        <a
